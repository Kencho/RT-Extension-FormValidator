...
sub Validate {
    ...
    foreach my $rule (@{$self->{ruleset}}) {
        my ($ok, $message) = $self->_ValidateWithRule($rule, $action, $queue_id, $form_data);
        if (!$ok) {
...
=head3 _ValidateWithRule($rule, $action, $queue_id, $form_data)
...
The rule to validate the form with.
